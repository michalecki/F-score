...
import os  # to access file list in the folder
import tabula  # to read in tables from pdf


def check_tables(summary, income_statement, balance_sheet, cash_flow, year, file):
    '''
    checks if retrieving correct values
    :param summary:
...
def calculate_f_score(file, year=2019):
    ...
    try:
        ...
        return file, 'wrong tables input reading'

    check_tables(summary, income_statement, balance_sheet, cash_flow, year, file)
    ...
    try:
        ...
        return file, 'roa index error'
    except ValueError:
        return file, 'roa value error'
...
def folder_loop(directory):
    ...
    :param directory: str, directory where the files are
    ...
    # modify the file name if calculating for different year than 2019
    try:
    ...
    for title in titles:
        if title not in calculated:
            # add here the year parameter if want to calculate earlier than 2019
